...
bind(mainMod .. " + A",
    exec(scriptsDir .. "/OverviewToggle.sh"),
    "desktop overview"
)
...
bind(mainMod .. " + T",
    exec(scriptsDir .. "/ThemeChanger.sh"),
    "Global theme switcher using Wallust"
)

bind(mainMod .. " + H",
    exec(scriptsDir .. "/KeyHints.sh"),
    "help / cheat sheet"
)
...
bind(mainMod .. " + ALT + E",
    exec(scriptsDir .. "/RofiEmoji.sh"),
    "emoji menu"
)

bind(mainMod .. " + S",
    exec(scriptsDir .. "/RofiSearch.sh"),
    "web search"
)
...
bind(mainMod .. " + ALT + O",
    exec(scriptsDir .. "/ChangeBlur.sh"),
    "toggle blur"
)

bind(mainMod .. " + SHIFT + G",
    exec(scriptsDir .. "/GameMode.sh"),
    "toggle game mode"
)

bind(mainMod .. " + ALT + L",
    exec(scriptsDir .. "/ChangeLayout.sh"),
    "toggle master/dwindle layout"
)

bind(mainMod .. " + ALT + V",
    exec(scriptsDir .. "/ClipManager.sh"),
    "clipboard manager"
)

bind(mainMod .. " + CTRL + R",
    exec(scriptsDir .. "/RofiThemeSelector.sh"),
    "rofi theme selector"
)

bind(mainMod .. " + CTRL + SHIFT + R",
...
bind(mainMod .. " + SHIFT + SPACE",
    function()
        hl.dispatch(hl.dsp.window.float({ action = "toggle" }))
        hl.dispatch(hl.dsp.window.resize({ x = 900, y = 600, relative = false }))
        hl.dispatch(hl.dsp.window.center())
    end,
    "Floating popup center"
)

-- There is no dedicated workspace-option dispatcher in the current Lua API.
bind(mainMod .. " + ALT + SPACE",
    exec("hyprctl dispatch workspaceopt allfloat"),
    "Float all windows"
)

bind(mainMod .. " + SHIFT + Return",
    exec(scriptsDir .. "/Dropterminal.sh " .. term),
    "DropDown terminal"
)
...
bind(mainMod .. " + CTRL + ALT + B",
    exec("pkill -SIGUSR1 waybar"),
    "toggle waybar on/off"
)

bind(mainMod .. " + CTRL + B",
    exec(scriptsDir .. "/WaybarStyles.sh"),
    "waybar styles menu"
)

bind(mainMod .. " + ALT + B",
    exec(scriptsDir .. "/WaybarLayout.sh"),
    "waybar layout menu"
)
...
bind(mainMod .. " + N",
    exec(scriptsDir .. "/Hyprsunset.sh toggle"),
    "toggle night light"
)
...
bind(mainMod .. " + SHIFT + M",
    exec(userScripts .. "/RofiBeats.sh"),
    "online music"
)

bind(mainMod .. " + W",
    exec(userScripts .. "/WallpaperSelect.sh"),
    "select wallpaper"
)

bind(mainMod .. " + SHIFT + W",
    exec(userScripts .. "/WallpaperEffects.sh"),
    "wallpaper effects"
)

bind("CTRL + ALT + W",
    exec(userScripts .. "/WallpaperRandom.sh"),
    "random wallpaper"
)
...
bind(mainMod .. " + SHIFT + K",
    exec(scriptsDir .. "/KeyBinds.sh"),
    "search keybinds"
)

bind(mainMod .. " + SHIFT + A",
    exec(scriptsDir .. "/Animations.sh"),
    "animations menu"
)

bind(mainMod .. " + SHIFT + O",
    exec(userScripts .. "/ZshChangeTheme.sh"),
...
for _, item in ipairs(monitorDirections) do
    bind(mainMod .. " + CTRL + " .. item.key,
        hl.dsp.workspace.move({ monitor = item.direction }),
        item.description
    )
end
...
bind("CTRL + ALT + L",
    exec(scriptsDir .. "/LockScreen.sh"),
    "lock screen"
)

bind("CTRL + ALT + P",
    exec(scriptsDir .. "/Wlogout.sh"),
    "powermenu"
)
...
bind(mainMod .. " + SHIFT + E",
    exec(scriptsDir .. "/Kool_Quick_Settings.sh"),
    "Quick settings menu"
)
...
bind("ALT + XF86AudioRaiseVolume",
    exec(scriptsDir .. "/Volume.sh --inc-precise"),
    "volume up precise",
    { locked = true, repeating = true }
)

bind("ALT + XF86AudioLowerVolume",
...
bind("XF86AudioMicMute",
    exec(scriptsDir .. "/Volume.sh --toggle-mic"),
    "toggle mic mute",
    { locked = true }
)

bind("XF86AudioMute",
    exec(scriptsDir .. "/Volume.sh --toggle"),
    "toggle mute",
    { locked = true }
)
...
bind("XF86RFKill",
    exec(scriptsDir .. "/AirplaneMode.sh"),
    "airplane mode",
    { locked = true }
)
...
bind("XF86AudioPause",
    exec(scriptsDir .. "/MediaCtrl.sh --pause"),
    "pause",
    { locked = true }
)
...
bind("XF86AudioNext",
    exec(scriptsDir .. "/MediaCtrl.sh --nxt"),
    "next track",
    { locked = true }
)

bind("XF86AudioPrev",
    exec(scriptsDir .. "/MediaCtrl.sh --prv"),
    "previous track",
    { locked = true }
)

bind("XF86AudioStop",
    exec(scriptsDir .. "/MediaCtrl.sh --stop"),
    "stop",
    { locked = true }
)
...
bind(mainMod .. " + Print",
    exec(scriptsDir .. "/ScreenShot.sh --now"),
    "screenshot now"
)

bind(mainMod .. " + SHIFT + Print",
    exec(scriptsDir .. "/ScreenShot.sh --area"),
    "screenshot (area)"
)

bind(mainMod .. " + CTRL + Print",
    exec(scriptsDir .. "/ScreenShot.sh --in5"),
    "screenshot in 5s"
)

bind(mainMod .. " + CTRL + SHIFT + Print",
    exec(scriptsDir .. "/ScreenShot.sh --in10"),
    "screenshot in 10s"
)

bind("ALT + Print",
    exec(scriptsDir .. "/ScreenShot.sh --active"),
    "screenshot active window"
)

bind(mainMod .. " + SHIFT + S",
    exec(scriptsDir .. "/ScreenShot.sh --swappy"),
    "screenshot (swappy)"
)
...
for _, item in ipairs(directions) do
    ...
    bind(mainMod .. " + ALT + " .. item.key,
        hl.dsp.window.swap({ direction = item.short }),
        "swap window " .. item.name
    )

    bind(mainMod .. " + " .. item.key,
        hl.dsp.focus({ direction = item.short }),
        "focus " .. item.name
    )
end
...
bind(mainMod .. " + Tab",
    hl.dsp.group.next(),
    "Change Group Forward"
)

bind(mainMod .. " + CTRL + Tab",
    hl.dsp.group.next(),
    "change active in group"
)
...
bind(mainMod .. " + mouse:272",
    hl.dsp.window.drag(),
    "move window",
    { mouse = true }
)

bind(mainMod .. " + mouse:273",
